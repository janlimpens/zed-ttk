...
// Scan content of a TT line directive (% already consumed) until newline or EOF
static bool scan_tt_line_directive_text(TSLexer *lexer) {
    lexer->mark_end(lexer);
    ...
    lexer->result_symbol = TT_LINE_DIRECTIVE_TEXT;
    return true;
}

// Scan content of a TT block directive ([% or [%# already consumed)
// Scans until %] is found. Stops BEFORE %] without consuming it.
// Uses mark_end semantics: advances past % to peek, but token ends before %.
static bool scan_tt_directive_content(TSLexer *lexer) {
    lexer->mark_end(lexer);

    while (lexer->lookahead != 0) {
        if (lexer->lookahead == '%') {
            // Peek ahead: advance past % to check for ]
            advance(lexer);
            if (lexer->lookahead == ']') {
                // Found %] - stop here. Content ends before the %.
                // mark_end was already set before advancing past %.
                lexer->result_symbol = TT_DIRECTIVE_CONTENT;
                return true;
            }
            // Not %] - include % in content, mark past it, continue
            lexer->mark_end(lexer);
            continue;
        }
        advance(lexer);
...
static bool scan_implicit_end_tag(Scanner *scanner, TSLexer *lexer) {
    ...
    // If we're looking at a TT directive, don't trigger implicit end tag
    if (lexer->lookahead == '%') {
        return false;
    }
...
static bool scan(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
    if (valid_symbols[TT_LINE_DIRECTIVE_TEXT]) {
        return scan_tt_line_directive_text(lexer);
    }

    if (valid_symbols[TT_DIRECTIVE_CONTENT]) {
        return scan_tt_directive_content(lexer);
    }
    ...
    switch (lexer->lookahead) {
        case '<':
            ...
            // Don't treat <% or <%== as start of an HTML tag
            // (same behavior as html-ep, for compatibility)
            if (lexer->lookahead == '%') {
                return false;
            }
...
void *tree_sitter_ttk_external_scanner_create() {
    Scanner *scanner = (Scanner *)ts_calloc(1, sizeof(Scanner));
    return scanner;
}

bool tree_sitter_ttk_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
    Scanner *scanner = (Scanner *)payload;
    return scan(scanner, lexer, valid_symbols);
}

unsigned tree_sitter_ttk_external_scanner_serialize(void *payload, char *buffer) {
    Scanner *scanner = (Scanner *)payload;
    return serialize(scanner, buffer);
}

void tree_sitter_ttk_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
    Scanner *scanner = (Scanner *)payload;
    deserialize(scanner, buffer, length);
}

void tree_sitter_ttk_external_scanner_destroy(void *payload) {
    Scanner *scanner = (Scanner *)payload;
    for (unsigned i = 0; i < scanner->tags.size; i++) {
        tag_free(&scanner->tags.contents[i]);
    }
